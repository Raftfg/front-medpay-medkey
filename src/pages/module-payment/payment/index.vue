    ...
    generateAndOpenPDF(facture) {
      ...
      factureService
        ...
        .then((res) => {
          ...
          console.log("this.signataires_data");
          console.log(this.signataires_data);
            ...
            if (this.facturesReference && this.facturesReference.length > 0) {
              ...
              for (var i = 0; i < this.facturesReference.length; i++) {
                ...
                var row = [
                  ...
                  this.formatNumber(this.facturesReference[i].quantite),
                  facture.type === 'A'
                    ? this.formatNumber(parseInt(this.facturesReference[i].prix))
                    : this.formatNumberDecimal(parseFloat(this.facturesReference[i].prix)),
                  facture.type == 'A'
                    ? this.formatNumber(amount)
                    : this.formatNumberDecimal(amount),
                ];
              ...
              let montantTotalVerse = this.facturesReference[0]['partial_amount'] !== null
                ? this.facturesReference[0]['partial_amount']
                : total - montantPriseEnCharge;
                
              if (this.facturesReference[0]['mode_payements_id'] == 8) {
                montantTotalVerse = 0;
              }
              ...
              rows.push([
                ...
                { content: facture.type === 'A' ? this.formatNumber(parseInt(montantTotalVerse)) : this.formatNumberDecimal(parseFloat(montantTotalVerse)) + " FCFA", colSpan: 1, styles: { fontStyle: 'bold', halign: 'right', fillColor: [44, 65, 86], textColor: [255, 255, 255] } },
              ]);
