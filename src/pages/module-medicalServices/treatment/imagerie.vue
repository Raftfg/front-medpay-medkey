       ...
       <div class="card">
         <div class="card-body">
          ...
          <div  v-if="showform" class="p-3 mt-2 bg-light"  style="font-size:11px; ">
            <form enctype="multipart/form-data"  @submit.prevent = "saveOperation()" class="">
              <div class="row">
            ...
            <div class="col-md-12 mt-4 bg-light">
             Image du résultat
             <b-form-file @change="selectFiles"></b-form-file>
           </div>
...
<div class="p-3">
  <p> <b>Actes d'imageries réalisées ( {{ operations.length }} )</b></p>
  <table class="table" width="90% !important">
    <tr v-for="operation in operations">
      <td>
        <b> {{operation.act_code}} - {{operation.actname}} </b> <br>
         <small> {{operation.reason}} </small>  <br>
  ...
  methods: {
   ...
   fetchMedicals() {
    ...
    fetchOperations() {
      ...
      medicalServices_services.getImagerieRecords(params)
      .then(
        (res) => {
    ...
    saveOperation() {
      ...
      formData.append('act_code',this.operation.act_code);
      formData.append('reason',this.operation.reason);
      formData.append('description',this.operation.description);
      formData.append('result',this.operation.result);
      formData.append('summary',this.operation.summary);
      formData.append('operator',this.operation.operator);
      formData.append('services_id',this.operation.services_id);
      formData.append('movments_id',this.operation.movments_id);
      formData.append('picture',this.picture);


      medicalServices_services.addImagerieRecords(formData,config)
      .then(
        (res) => {
