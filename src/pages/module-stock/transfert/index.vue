  ...
  <section class="container-scroller">
    <div class="row">
      <div class="col-lg-12 grid-margin">
        <h3 class="bg-success card-title text-white rounded p-2 mb-2 text-center" style="background-color: #0b5d3f; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); ">Liste des transferts</h3>
        <div class="card">
          <div class="card-body">
            <div class="d-flex flew-row justify-content-between" id="fixingIssue">
              <router-link to="/transfert/create" class="">
                <button
                  type="submit"
              ...
              <div class="row">
                <div class="form-group form-controls col-12">
                  <select 
                    ...
                    @change="getStockTransfers(filter.store_id)"
            ...
            <div class="table-responsive mt-4">
              ...
              >
                ...
                <tbody>
                  <tr v-for="(supply, index) in stockTransfers" :key="supply.uuid">
                    <td>{{ index + 1 }}</td>
                    <td>{{ supply.created_at }}</td>
                    <td>{{ supply.stock_source.name }}</td>
                    <td>{{ supply.stock_destination.name }}</td>
                    <!-- <td>{{ supply.products_number }}</td> -->
...
export default {
  ...
  mounted() {
    ...
    .then((res) => {
      ...
      //get all stockTransfers related to the first store
      storeService.
      getStockTransfers(res.data.data[0].uuid)
      .then((res) => {
        this.stockTransfers = res.data.data;

        this.$nextTick(() => {
          const table = $(this.$refs.myTable).DataTable({
            "initComplete": function (settings, json) {
                //When the data is fully loaded reduce the margin bottom
                $('#fixingIssue').css('margin-bottom', '-50px');
              },
            dom:
  ...
  methods: {
    getStockTransfers(uuid)
    {
      storeService
      .getStockTransfers(uuid).then((res) => {
        this.stockTransfers = res.data.data;
        const table = $(this.$refs.myTable).DataTable();
        table.destroy();
        ...
        this.$nextTick(() => {
          const newTable = $(this.$refs.myTable).DataTable({
            "initComplete": function (settings, json) {
              // When the data is fully loaded reduce the margin bottom
    ...
    details(uuid) {
      this.$router.push("/transfert/" + uuid + "/products");
    },
  },
};
</script>
