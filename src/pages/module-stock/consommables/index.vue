  ...
  <section class="tables">
    <div class="row">
      <div class="col-lg-12 grid-margin">
        <h3 class="bg-success card-title text-white rounded p-2 mb-2 text-center" style="background-color: #0b5d3f; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); ">Liste des consommables</h3>
        <div class="card">
          <div class="card-body">
            <div class="d-flex flew-row justify-content-between" id="fixingIssue" style="margin-bottom: 5px;">
              <router-link to="/consommables/create" class="">
                <button
                  type="submit"
            ...
            <div class="table-responsive">
              ...
              >
                ...
                <tbody>
                  <tr v-for="(consumable, index) in consumables" :key="consumable.uuid">
                    <td>{{ index + 1 }}</td>
                    <td>{{ consumable.code }}</td>
                    <td>{{ consumable.name }}</td>
                    <td>{{ consumable.category.name }}</td>
                    <td>{{ consumable.brand }}</td>
                    <td>{{ consumable.sale_unit.name }}</td>
                    <td>{{ consumable.conditioning_unit.name }}</td>
                    <td class="text-right" width="50px">
                      <div class="btn-group btn-group-sm" role="group">
                        ...
                        >
                          <button
                            @click="detailsProduct(consumable.uuid)" class="dropdown-item font-weight-bold"
                            style="color: black;">
...
export default {
  ...
  methods: {
    detailsProduct(uuid)
    {
      this.$router.push("/consommables/" + uuid + "/details");
    },
    update(uuid) {
        this.$router.push("/consommables/update/" + uuid);
    },
  },
};
</script>
