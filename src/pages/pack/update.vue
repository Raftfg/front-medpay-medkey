        ...
        <div class="card">
          <div class="card-body">
            <h3 class="card-title text-white rounded p-3 mb-4 text-center"
              ...
              Modifier des packs à une assurance
            </h3>
...
export default {
  ...
  data() {
    return {
      ...
      insurances_id: "",
      pack: {
        designation: "", // Ajoutez ces propriétés avec des valeurs par défaut
        percentage: ""
      }
    };
  },
  mounted() {
    ...
    //   this.selectedAssurance = res.data.data.insurances.uuid;
    //   console.log(this.pack);
    //   console.log(this.pack.designation);

    // });

    packService.getPacksOfInsuranceByPack(this.uuid).then((res) => {
      this.packs = res.data.data;
      this.selectedAssurance = res.data.data[0].insurances.uuid;
      console.log(this.packs);
    });

    // // Récupérer les packs associés à l'assurance
    // packService.getPackByAssurance(this.selectedAssurance).then((res) => {
    //   this.packs = res.data;
    // }).catch((error) => {
    //   console.error(error);
    // });
