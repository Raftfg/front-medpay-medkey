        ...
        <div class="card">
          <div class="card-body">
            <h4 class=" text-white rounded p-3 text-center font-weight-bold"
              ...
              Liste des paiements remboursable pour le patient
              <span style="text-transform: uppercase">{{ patient.lastname }} {{ patient.firstname }}</span>
            </h4>
            ...
            <div class="table-responsive py-1">
              <table id="order-listing" ref="myTableFactures" class="table table-bordered">
                ...
                <tbody>
                  <tr class="row-hover" v-for="( paymentDetail, index) in  paymentDetails" :key="paymentDetail.id">
                    <td>{{ index + 1 }}</td>
                    ...
                    <td>{{ paymentDetail.prix }}</td>
                    <td>{{ paymentDetail.quantite }}</td>
                    ...
                    <td class="text-right" width="50px">
                      <div class="btn-group btn-group-sm" role="group">
                        ...
                        <div class="dropdown-menu action-button-div" aria-labelledby="btnGroupDrop1"
                          ...
                          <a  @click="processRefund(paymentDetail)">
...
export default {
  ...
  mounted() {
    ...
    patientService.getDetailPatient(this.id).then((res) => {
      console.log("ok")
      console.log(res);
    ...
    remboursementService.getPaymentDetails(this.id).then((res) => {
  ...
  methods: {
   // Ajoutez cette méthode de remboursement
    async processRefund(paymentDetail) {
      try {
        // Appelez la fonction de remboursement du service
        const response = await remboursementService.processRefund(this.patient.id, paymentDetail.id);

        // Affichez un message de succès ou effectuez d'autres opérations si nécessaire
