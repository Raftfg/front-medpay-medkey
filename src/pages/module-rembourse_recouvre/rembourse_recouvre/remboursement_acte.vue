    ...
    async genererFactureRemboursement() {
      ...
      if (this.pourcentageRemboursement !== null && this.selectedFacture !== null) {
        try {
          ...
          // // Appeler la fonction pour récupérer le nom et le prénom du caissier
          // const response = await remboursementService.generateFactureWithCaissierName(this.selectedFacture.id);
          // const caissierName = response.data.facture.caissier_name; // Utilisez la casse correcte
          // const caissierPrenom = response.data.facture.caissier_prenom; // Utilisez la casse correcte

          // // Ajouter les zones de signature
          // const caissierSignature = "Caissier";
          // const patientSignature = "Patient";
          // pdf.text(`Caissier: ${caissierName} ${caissierPrenom}`, 15, 94);
          // pdf.text(`Patient: ${patientName}`, 15, 80);
          ...
          // Calculer la position pour aligner les signatures
          const signaturesY = pdf.autoTable.previous.finalY + 30;
          const signatureSpacing = 10;  // Espace entre chaque ligne de signature
          ...
          // Ajouter les textes avec les noms et l'espace supplémentaire
          pdf.text(`Caissier(e)`, caissierSignatureX, signaturesY);
          pdf.text(`${caissierName} ${caissierPrenom}`, caissierSignatureX, signaturesY + signatureSpacing);

          pdf.text(`Patient`, patientSignatureX, signaturesY);
          pdf.text(`${patientName}`, patientSignatureX, signaturesY + signatureSpacing);
    ...
    async genererFactureRemboursementMedicament() {
      ...
      if (this.selectedFactureMedicament !== null) {
        try {
          ...
          // Appeler la fonction pour récupérer le nom et le prénom du caissier
          const response = await remboursementService.generateFactureWithCaissierName(this.selectedFactureMedicament.id);
          const caissierName = response.data.facture.caissier_name; // Utilisez la casse correcte
          const caissierPrenom = response.data.facture.caissier_prenom; // Utilisez la casse correcte
